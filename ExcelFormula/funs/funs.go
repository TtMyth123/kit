...
func FLOOR(input interface{}) (float64, error) {
	return math.Floor(input.(float64)), nil
}

// add ty
func FLOOR2(input1 interface{}, input2 interface{}) (float64, error) {
	return math.Floor(input1.(float64) * input2.(float64)), nil
}

func ROUND(input interface{}, precision float64) (float64, error) {
	pow := math.Pow(10, precision)
	return math.Round(input.(float64)*pow) / pow, nil
}
...
func SUM2(input1 interface{}, input2 interface{}) (float64, error) {
	return SUMn(input1, input2)
}

func SUMn(inputs ...interface{}) (float64, error) {
	sum := 0.0
...
func POWER(num interface{}, power interface{}) (float64, error) {
	switch num.(type) {
	case float64:
		switch power.(type) {
		case float64:
			return math.Pow(num.(float64), power.(float64)), nil
		case int:
			return math.Pow(num.(float64), float64(power.(int))), nil
		}
	case int:
		switch power.(type) {
		case float64:
			return math.Pow(float64(num.(int)), power.(float64)), nil
		case int:
			return math.Pow(float64(num.(int)), float64(power.(int))), nil
		}
	}
	return 0.0, nil
}
...
func MATCH(value interface{}, lookupRange interface{}, matchType int) (int, error) {
...
func COUNTIF(lookupRange interface{}, referenceValue interface{}) (count float64, err error) {
	count = 0
	err = nil
	switch lookupRange.(type) {
	case [][]interface{}:
