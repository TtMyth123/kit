...
import (
	"errors"
	"testing"
	. "ttmyth123/kit/ExcelFormula/funs"
)
...
func TestFLOOR(t *testing.T) {
	if result, _ := FLOOR(1.1); result != 1 {
		t.Errorf("Expected: 1\tActual:%v", result)
	}
}

func TestSUM(t *testing.T) {
	if result, _ := SUM(2.5); result != 2.5 {
		t.Errorf("Expected: 1\tActual:%v", result)
	}

	arr1D := []float64{1, 2, 3, 4, 5}
	if result, _ := SUM(arr1D); result != 15 {
		t.Errorf("Expected: 15\tActual:%v", result)
	}

	arr2D := [][]float64{{1, 2}, {3, 4}, {5, 6}}
	if result, _ := SUM(arr2D); result != 21 {
		t.Errorf("Expected: 21\tActual:%v", result)
	}
}

func TestPOWER(t *testing.T) {
	if result, _ := POWER(2.0, 3.0); result != 8 {
...
func TestCOUNTIFfloat64(t *testing.T) {
	...
	if result, _ := COUNTIF(lookupRange, 1.0); result != 1.0 {
		t.Errorf("Expected: 1\tActual:%v", result)
	}
	...
	if result, _ := COUNTIF(lookupRange2D, 1.0); result != 1.0 {
		t.Errorf("Expected: 1\tActual:%v", result)
	}
	...
	if result, _ := COUNTIF(lookupRange2D, 2.0); result != 0.0 {
...
func TestCOUNTIFstring(t *testing.T) {
	...
	if result, _ := COUNTIF(lookupRange, "hello"); result != 1.0 {
		t.Errorf("Expected: 1\tActual:%v", result)
	}
	...
	if result, _ := COUNTIF(lookupRange2D, "hello"); result != 1.0 {
	...
	if result, _ := COUNTIF(lookupRange2D, "Cartoon"); result != 2.0 {
