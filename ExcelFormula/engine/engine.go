	...
	if invoke.fn == "IDENTITY" {
		...
		case int:
			...
			case error:
				...
				break
			case int:
				ret = g.logicalInt(invoke.fn, operand1.(int), operand2.(int))
				break
			case float64:
				ret = g.logicalFloat64(invoke.fn, float64(operand1.(int)), operand2.(float64))
				break
			case string:
...
func (g *Engine) logicalInt(fn string, operand1 int, operand2 int) bool {
	if fn == ">" {
...
func (g *Engine) callFunc(node *f1F.Node) (err error) {
	...
	if !strings.Contains("IDENTITY+-*/>=<=<>", fn) && !funs.Exists(fn) {
		println(fmt.Sprintf("5函数未实现:%s", fn))
		err = errors.New(fmt.Sprintf("6函数未实现:%s", fn))
		return
	}
